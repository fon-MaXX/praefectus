...
func (p *ScalePool) tryDownscaleWorkers() {
	...
	for _, command := range p.workersRegistry.storage {
		...
		log.WithFields(log.Fields{"pool": p.id, "idle": workerIdleStatePercentage}).Debug("-----=======Worker idle===========-----")
	...
	log.WithFields(log.Fields{"pool": p.id, "idle": workerPercentage, "remove": removeWorkers}).Debug("-----=======Pool idle===========-----")
}
...
func (p *ScalePool) calculateWorkersBusiness() uint8 {
	...
	for _, command := range p.workersRegistry.storage {
		...
		log.WithFields(log.Fields{"pool": p.id, "business": res}).Debug("-----=======Worker business===========-----")
	}

	workerPercentage := math.Round(float64(workersBusyness) / float64(activeWorkers))
	log.WithFields(log.Fields{"pool": p.id, "business": workerPercentage, "activeWorkers": activeWorkers}).Debug("-----=======Pool business===========-----")
